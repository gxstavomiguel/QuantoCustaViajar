...
public class RelatorioActivity extends AppCompatActivity {
    ...
    private void loadRelatorio() {
        ...
        String query = "SELECT destino,  \n" +
                ...
                "                       data_fim,  \n" +
                "                       qt_pessoas, \n" +
                "                       round((coalesce((gasolina_total_km / (CASE WHEN gasolina_media_km_por_litro = 0 THEN 1 ELSE gasolina_media_km_por_litro END) * gasolina_custo_medio_por_litro / CASE WHEN gasolina_total_veiculos = 0 THEN 1 ELSE gasolina_total_veiculos END), 0) +    \n" +
                "                       coalesce((tarifa_custo_total * QT_PESSOAS + TARIFA_ALUGUEL_VEICULO), 0) +  \n" +
                "                       coalesce((refeicao_custo_por_refeicao * refeicao_num_refeicoes_por_dia * REFEICAO_CUSTO_POR_REFEICAO * ((julianday(strftime('%Y-%m-%d', substr(data_fim, 7, 4) || '-' || substr(data_fim, 4, 2) || '-' || substr(data_fim, 1, 2))) - \n" +
                "                        julianday(strftime('%Y-%m-%d', substr(data_inicio, 7, 4) || '-' || substr(data_inicio, 4, 2) || '-' || substr(data_inicio, 1, 2)))))), 0) +  \n" +
                "                       coalesce((hospedagem_custo_por_noite * hospedagem_total_noites * HOSPEDAGEM_TOTAL_QUARTOS), 0) + \n" +
                "                       coalesce((SELECT COALESCE(sum(custo), 0) FROM ENTRETENIMENTO E WHERE E.IDVIAGEM = VIAGEM.ID), 0)), 2) AS custo_total  \n" +
                "                       FROM viagem  \n" +
